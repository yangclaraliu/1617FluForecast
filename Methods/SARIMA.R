...
for(h in 1:10){
  ...
  #plot(flu$X..WEIGHTED.ILI,type='l')
  start <- which(flu$WEEK==40)
  end <- c(which(flu$WEEK==39),nrow(flu))
  flu["season"]<-NA;for(i in 1:length(start)){flu$season[start[i]:end[i]]<-i}
  #plot(flu$season)
  flu$X..WEIGHTED.ILI <- ts(as.numeric(flu$X..WEIGHTED.ILI),freq=52)
  #acf(flu$X..WEIGHTED.ILI)
  #pacf(flu$X..WEIGHTED.ILI)
  #nonstationary behavior
  #tsdisplay(flu$X..WEIGHTED.ILI)
  #tsdisplay(diff(flu$X..WEIGHTED.ILI))
  flu$X..WEIGHTED.ILI[flu$X..WEIGHTED.ILI==0] <- NA
  model <- Arima(log(flu$X..WEIGHTED.ILI),c(3,1,0),c(0,1,2))
  ...
  for(i in 1:nsim){
    repeat{
      ...
      #check 2 things. (2)
      peaktime_temp <- tail(flu$WEEK,1)+peakwindow(simulated_data)$smd.max.index
      peaktime_index <- peakwindow(simulated_data)$smd.max.index
      if(peaktime_temp>52) peaktime_temp <- peaktime_temp-52
      toolate <- ( peaktime_temp>=15 & peaktime_temp < 40)
    ...
    peaksize[i] <- simulated_data[peaktime_index]
    ...
    if (length(onsettime_temp)!=0)  {onsettime[i] <- epidemic_week[min(onsettime_temp)]} else {onsettime[i] <- NA}
...
##############Save the results by HHS first###########################
  ...
  points(tail(flu$X..WEIGHTED.ILI,week_in),col="green",pch=20)
  points(y=exp(forecast(model,52-week_in)$mean[1:4]),x=c((week_in+1):(week_in+4)),pch=20,col="red")
  lines(tail(flu$X..WEIGHTED.ILI,week_in),col="green",lwd=2)
  lines(y=c(tail(flu$X..WEIGHTED.ILI,1),exp(forecast(model,52-week_in)$mean[1:4])),x=c((week_in):(week_in+4)),col="red",lty=2,lwd=2)
...
if(length(onsettime_point)!=0) onsettime_point <- min(onsettime_point) else onsettime_point <- NA
onsettime_point <- epidemic_week[onsettime_point]
point_res <- c(onsettime_point,epidemic_week[which.max(c(tail(exp(model$x),week_in),exp(forecast(model,52-week_in)$mean)))],max(c(tail(exp(model$x),week_in),exp(forecast(model,52-week_in)$mean))))
# point_res
res <- c(point_res[1],draw.onset.sim(onsettime)[,2],point_res[2],draw.peakweek.sim(peaktime)[,2],point_res[3],draw.peak.sim(peaksize)[,3],exp(forecast(model,52-week_in)$mean[1]),draw.wk.sim(sim_res[1,])[,3],exp(forecast(model,52-week_in)$mean[2]),draw.wk.sim(sim_res[2,])[,3],exp(forecast(model,52-week_in)$mean[3]),draw.wk.sim(sim_res[3,])[,3],exp(forecast(model,52-week_in)$mean[4]),draw.wk.sim(sim_res[4,])[,3])
